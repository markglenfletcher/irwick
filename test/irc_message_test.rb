...
class IrcMessageTest < Minitest::Test
  ...
  def test_parsed_message_behaves_correctly
    ...
    assert_irc_message_contains IrcMessage.parse(message), :type => :away, :message => 'Gone to lunch.  Back in 5'

    message = ':WiZ AWAY'
    assert_irc_message_contains IrcMessage.parse(message), :type => :away, :user => 'WiZ'

    # REHASH
    message = 'REHASH'
    assert_irc_message_contains IrcMessage.parse(message), :type => :rehash

    # RESTART
    message = 'RESTART'
    assert_irc_message_contains IrcMessage.parse(message), :type => :restart

    # SUMMON
    message = 'SUMMON jto'
    assert_irc_message_contains IrcMessage.parse(message), :recipient => 'jto'
    ...
    assert_irc_message_contains IrcMessage.parse(message), :user => 'csd.bu.edu', :message => "Connect '*.uiuc.edu 6667' from Joshua"
